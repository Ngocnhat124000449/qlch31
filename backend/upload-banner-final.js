    ...
    const req = protocol.request(reqOptions, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          else reject(new Error(`Parse error: ${data}`));
        }
      });
    });
...
/**
 * Upload helper - create proper multipart body
 */
...
async function main() {
  try {
    console.log("🚀 Banner Upload to Cloudinary\n");
    ...
    const fields = {
      ten: `Banner Cloudinary Upload - ${new Date().toISOString().slice(0, 10)}`,
      mota: "Banner test upload to Cloudinary and save URL to database",
    ...
    // 5. Verify via GET /api/banners
    console.log("\n4️⃣  Verifying banner in database...");
    ...
    console.log(
      `\n✅ Complete! Banner uploaded to Cloudinary and saved to database.`,
    );
  } catch (err) {
    console.error("\n❌ Error:", err.message);
    try {
      fs.unlinkSync(path.join(process.cwd(), "temp-banner.jpg"));
    } catch (e) {}
    process.exit(1);
  }
}
